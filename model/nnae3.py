...
class Deep_Autoencoder(object):
    ...
    def global_nn_constructing(self, sess, saver, training_with_regu, regu1, regu2, data):
        last_layer = self.x
        for weight,bias,j in zip(self.W_list, self.encoding_b_list, range(len(self.dim_list))):
            if j == len(self.dim_list) - 1:
                hidden = tf.matmul(last_layer, weight) + bias
        ...
        sess.run(tf.global_variables_initializer())
        init_recon_error = self.recon_cost.eval(session=sess, feed_dict={self.input_x: data})
        init_regu1 = self.regularization_cost_1.eval(session=sess, feed_dict={self.input_x: data})
        init_regu2 = self.regularization_cost_2.eval(session=sess, feed_dict={self.input_x: data})
        init_total = self.cost.eval(session=sess, feed_dict={self.input_x: data})
        saver.restore(sess, self.save_path) 
        return init_recon_error, init_regu1, init_regu2, init_total
...
def NNAEDimReduction(data, layersNum, compressionRate, iteration, learning_rate, regu1, regu2):
    ...
    # layers_sizes = [data.shape[-1], 60,30,10,3]
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            i1,i2,i3,i4 = ae.global_nn_constructing(sess=sess, saver=saver, training_with_regu=True, regu1=regu1, regu2=regu2, data=data)
            f1,f2,f3,f4 = ae.fit(X=data, saver=saver,
                        sess=sess,
                        iteration=iteration,
                        batch_size=batch_size,
                        )
    ...
    return hidden, recon_error, i1, i2, i3, f1, f2, f3
