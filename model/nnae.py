...
class Deep_Autoencoder(object):
    def __init__(self, sess, input_dim_list, num_neighbours, learning_rate):
        ...
        self.input_x = tf.placeholder(tf.float32,[None,self.num_neighbours+1,self.dim_list[0]])
        self.x = self.input_x[:,0,:]
        self.neighbors = self.input_x[:,1:,:]
        self.saver = tf.train.Saver()

    def calculate_pointstructure_maintaining_regularization(self, pretraining):
        i = self.i
        last_layer_neighbors = tf.reshape(self.neighbors, (-1,self.dim_list[0]))
        if pretraining:
    ...
    def pre_nn_constructing(self, sess, i, saver, training_with_regu, regu_coef):
        self.i = i
        ...
        self.regularization_cost = regu_coef * self.calculate_pointstructure_maintaining_regularization(True)
        self.cost = self.recon_cost + self.regularization_cost
    ...
    def global_nn_constructing(self, sess, saver, training_with_regu, regu_coef):
        last_layer = self.x
        for weight,bias,j in zip(self.W_list, self.encoding_b_list, range(len(self.dim_list))):
            if j == len(self.dim_list) - 1:
                hidden = tf.matmul(last_layer, weight) + bias
        ...
        self.recon = last_layer
        self.recon_cost = 2e2 * tf.reduce_mean(tf.square(self.x - self.recon))
        self.regularization_cost = regu_coef * self.calculate_pointstructure_maintaining_regularization(False)
        self.cost = self.recon_cost + self.regularization_cost
        if not training_with_regu:
            self.cost = 2e2 * tf.reduce_mean(tf.square(self.x - self.recon))
    ...
    def fit(self, X, saver, sess, iteration=50, batch_size=133, saving=True):
        ...
        for i in range(iteration):
            ...
            if i%20==0:
                ...
                g = self.regularization_cost.eval(session=sess, feed_dict={self.input_x:X})
                h = self.cost.eval(session = sess,feed_dict = {self.input_x: X})
                print ("    iteration : ", i, ", recon_cost : ", f, ", regu : ", g, ", cost : ", h)
        if saving:
...
def NNAEDimReduction(data, layersNum, compressionRate, iteration, learning_rate, regulationCoefficient):
